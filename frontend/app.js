...
let currentRunId = null, pollTimer = null, eventSource = null, generationReady = false, imageReady = false, providerCatalog = {}, styleCatalog = null, isAuthenticated = false, authenticatedUsername = '', pendingProtectedView = null;
...
function closeAuthPrompt(){document.body.classList.remove('auth-prompt');$('#loginGate').setAttribute('aria-hidden','true');pendingProtectedView=null}
...
$('#loginForm').onsubmit=async event=>{event.preventDefault();const button=$('#loginButton'),error=$('#loginError'),targetView=pendingProtectedView;error.textContent='';button.disabled=true;try{const response=await fetch(`${API}/auth/login`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:$('#loginUsername').value.trim(),password:$('#loginPassword').value})});const result=await response.json();if(!response.ok)throw new Error(result.detail||'登录失败');setAuthenticated(true,result.username);pendingProtectedView=null;await initializeApp();if(targetView)showView(targetView);toast(`${result.username} 已登录，生成服务已解锁`)}catch(loginError){error.textContent=loginError.message}finally{button.disabled=false}};
