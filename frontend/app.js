...
let currentRunId = null, pollTimer = null, eventSource = null, generationReady = false, imageReady = false, providerCatalog = {}, styleCatalog = null;
...
async function bootstrapAuth(){try{const response=await fetch(`${API}/auth/session`);const session=await response.json();setAuthenticated(Boolean(session.authenticated));if(session.authenticated)await initializeApp()}catch{setAuthenticated(false);$('#loginError').textContent='认证服务暂不可用'}}
bootstrapAuth();
