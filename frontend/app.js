...
function renderTemplateGrid(){
  ...
  $('#templateGrid').innerHTML=items.map(item=>`<article class="template-card"><img src="${esc(item.cover)}" alt="${esc(item.name)}"><div><span class="template-ratio">${esc(item.category)} · ${esc(item.aspect_ratio)}</span><h3>${esc(item.name)}</h3><p>${esc(item.summary)}</p><button class="secondary template-use" data-template="${esc(item.id)}" data-category="${esc(item.category_id)}"><span>在创作页使用</span><b>→</b></button></div></article>`).join('');
  $$('.template-use').forEach(button=>button.onclick=()=>useTemplateInCreation(button.dataset.template,button.dataset.category));
}
function useTemplateInCreation(templateId,categoryId){showView('generate');$('#creationImageCategory').value=categoryId;renderCreationImageTemplates(templateId);$('#creationOutputType').value='image';renderPipeline('image');refreshGenerateAvailability();document.querySelector('.generation-style-controls').scrollIntoView({behavior:'smooth',block:'center'});toast('模板已带入创作页，请上传衣服后统一生成')}
...
function refreshGenerateAvailability(){const outputType=$('#creationOutputType')?.value||'image',ready=outputType==='image'?imageReady:generationReady,button=$('#generateButton'),notice=$('#generationNotice');button.disabled=isAuthenticated&&!ready;$('#creationVideoStyleLabel').classList.toggle('style-disabled',outputType==='image');$('#generateButtonLabel').textContent=outputType==='image'?'按当前衣服生成所选图片模板':'按当前衣服和所选风格生成 18S 视频';notice.textContent=!isAuthenticated?'上传衣服并点击生成后，登录账号即可自动继续当前创作。':outputType==='image'?'仅调用独立图片 API；图片模板产物与生成清单单独保存，不调用视觉或视频 API。':'图片模板控制关键帧，视频风格独立控制动态镜头；生成前校验三个 API。';notice.classList.toggle('ready',!isAuthenticated||ready)}
